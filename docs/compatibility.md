...
| `CommandRequest.typed_arguments` (`repeated CommandArgumentEntry`) | `CommandRequest.arguments` map keys (`device_id`, `device_ids`, `activation_id`) | 2026-05-05 (schema and shared Go/Dart golden fixture cover typed + legacy coexistence; producer/consumer wiring remains command-path dependent) | After typed producer/consumer wiring lands plus two tagged releases |

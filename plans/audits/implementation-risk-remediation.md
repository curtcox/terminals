...
- 2026-04-25: Completed a Stage 8 monitoring-tier increment by declaring explicit foreground-only monitor support (`edge.operators=monitor.tier.foreground_only`) in client capability probes, updating client probe/widget regressions to lock tier persistence without synthetic lifecycle operators, and documenting server placement semantics for monitor support tiers.
- 2026-04-25: Completed a Stage 7 permission-hardening increment by aligning declared Android/iOS permission metadata with client docs (`INTERNET` + `CHANGE_WIFI_MULTICAST_STATE` on Android and `NSLocalNetworkUsageDescription` + `NSBonjourServices` on iOS), expanding platform-readiness regression coverage for those keys, and documenting Linux/Windows desktop media-permission limits plus deterministic denied-permission failure behavior.
- 2026-04-25: Implemented Stage 9 native alert delivery parity by routing explicit `ConnectResponse.notification` alerts through a new client alert-delivery service that triggers best-effort OS notifications on non-web platforms while preserving spoken fallback and existing in-app status handling semantics; added deterministic unit coverage for notification/speech routing behavior.
- 2026-04-25: Replaced remaining audio-visualizer placeholder semantics with deterministic stream-attached state (`Waiting for media`/`Attached`) wired to media-engine attach signals, including explicit progress-bar determinate/indeterminate behavior and widget regression coverage for start/stop transitions.

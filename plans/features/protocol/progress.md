...
## Protocol Evolution Rules (2026-05-05, lint cleanup + plan complete)

- Fixed `exhaustive` lint warnings on all typed-enum switches introduced during the migration phase: added explicit `UNSPECIFIED` cases with return-to-legacy-fallback semantics to `FlowState` (`control_stream.go`), `PointerAction`, `TouchAction`, `WebRTCSignalType` (`generated_proto_adapter.go`), `StreamOrigin` (`stream_routing.go`), and `CanvasOpKind` (`canvas_draw_ops.go`, `canvas.go`).
- Added export doc-comments for all `CanvasOpKind` constants (`CanvasOpLineKind` through `CanvasOpPathKind`) to satisfy `revive` exported-const rule.
- Preallocated the `typedKeys` slice in the command result adapter test to satisfy `prealloc`.
- Suppressed the `staticcheck` deprecated-field warning in `assertDeprecatedRegisterDevice` with a `//nolint:staticcheck` comment explaining the intentional backward-compatibility test.

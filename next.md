...
3. Replace `WireProtoAdapter` in server/grpc wiring with generated-protobuf adapter using the same structured semantics already modeled in wire types (`CommandRequest`, `CommandResult`, `ControlError`, `error_code`, deterministic map payload encoding).

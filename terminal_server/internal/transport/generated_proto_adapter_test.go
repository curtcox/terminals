...
func TestGeneratedProtoAdapterFromInternalHelloAndCapabilityAck(t *testing.T) {
	...
	if resp.GetCapabilityAck().GetSnapshotApplied() {
		t.Fatalf("snapshot_applied = true, want false")
	}
	...
	if err != nil {
		t.Fatalf("FromInternal(capability_ack snapshot_applied=true) error = %v", err)
	}
	resp, ok = env.(*controlv1.ConnectResponse)
	if !ok {
		t.Fatalf("unexpected proto envelope type %T", env)
	...
	if !resp.GetCapabilityAck().GetSnapshotApplied() {
		t.Fatalf("snapshot_applied = false, want true")
	}
	if resp.GetCapabilityAck().GetAcceptedGeneration() != 3 {
		t.Fatalf("accepted_generation = %d, want 3", resp.GetCapabilityAck().GetAcceptedGeneration())
	}
}
...
func TestGeneratedProtoAdapterToInternalSensorAndStreamReady(t *testing.T) {
	...
	webrtcEnumMsg, err := adapter.ToInternal(&controlv1.ConnectRequest{
		Payload: &controlv1.ConnectRequest_WebrtcSignal{
			WebrtcSignal: &controlv1.WebRTCSignal{
				StreamId:       "stream-8",
				SignalTypeEnum: controlv1.WebRTCSignalType_WEB_RTC_SIGNAL_TYPE_ICE_CANDIDATE,
...
func TestGeneratedProtoAdapterFromInternal(t *testing.T) {
	...
	if got := resp.GetWebrtcSignal().GetSignalTypeEnum(); got != controlv1.WebRTCSignalType_WEB_RTC_SIGNAL_TYPE_ANSWER {

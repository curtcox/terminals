...
func runApp(args []string, stdout, stderr io.Writer) int {
	...
	switch command {
	case "new":
		return runAppNew(name, root, stdout, stderr)
	case "check", "load":
		return runAppCheck(command, root, stdout, stderr)
	case "test":
		return runAppTest(name, root, stdout, stderr)
	case "reload", "rollback":
		return runAppAdminCommand(command, name, root, stdout, stderr)
	case "logs":
		return runAppLogs(name, stdout, stderr)
	case "trace":
		return runAppTrace(stdout, stderr)
	default:
...
func runAppNew(name, root string, stdout, stderr io.Writer) int {
	if err := os.MkdirAll(root, 0o755); err != nil {
		return fail(stderr, err)
	}
	manifest := fmt.Sprintf("name = %q\nversion = %q\nlanguage = %q\ndescription = %q\n",
		name, "0.1.0", appruntime.LanguageTALV1, "Generated by term app new")
	if err := writeIfMissing(filepath.Join(root, "manifest.toml"), []byte(manifest)); err != nil {
		return fail(stderr, err)
	}
...
func runAppCheck(command, root string, stdout, stderr io.Writer) int {
...
func runAppTest(name, root string, stdout, stderr io.Writer) int {
	runtime := newAppRuntime()
	pkg, err := runtime.LoadPackage(context.Background(), root)
	if err != nil {
		return fail(stderr, err)
	...
	passed, err := printTALTestResults(stdout, results)
	if err != nil {
		return fail(stderr, err)
	}
...
func printTALTestResults(stdout io.Writer, results []talTestResult) (int, error) {
	passed := 0
...
func runAppAdminCommand(command, name, root string, stdout, stderr io.Writer) int {
	result, err := postAdminAppCommand(command, name)
	if err != nil {
		if command == "reload" {
			return runAppLocalReload(name, root, stdout, stderr)
		}
		return fail(stderr, err)
	}
...
func runAppLocalReload(name, root string, stdout, stderr io.Writer) int {
	runtime := newAppRuntime()
	if _, loadErr := runtime.LoadPackage(context.Background(), root); loadErr != nil {
		return fail(stderr, loadErr)
	}
	_, changed, reloadErr := runtime.ReloadPackage(context.Background(), name)
	if reloadErr != nil {
	...
	if err := writef(stdout, "reload ok: %s %s (local runtime)\n", name, status); err != nil {
		return fail(stderr, err)
	}
	return 0
}

func runAppLogs(name string, stdout, stderr io.Writer) int {
	payload, err := getAdminJSON("/admin/api/apps")
	if err != nil {
		return fail(stderr, err)
	}
	row := findAppRow(payload, name)
	if row == nil {
...
func findAppRow(payload map[string]any, name string) map[string]any {
	apps, _ := payload["apps"].([]any)
	for _, app := range apps {
		row, ok := app.(map[string]any)
		if ok && strings.TrimSpace(toString(row["name"])) == name {
			return row
		}
	}
	return nil
...
func runAppTrace(stdout, stderr io.Writer) int {
...
func runLogs(args []string, stdout, stderr io.Writer) int {
	...
	rest, err := parseLogOptions(rest, &opts)
	if err != nil {
		return fail(stderr, err)
	}
	if opts.since != "" {
	...
	switch cmd {
	case "tail":
		return runLogsTail(opts, rest, stdout, stderr)
	case "search":
	...
	case "trace":
		return runLogsTrace(opts, rest, stdout, stderr)
	case "activation":
		return runLogsActivation(opts, rest, stdout, stderr)
	case "stats":
		return runLogsStats(opts, rest, stdout, stderr)
	default:
...
func parseLogOptions(args []string, opts *logsOptions) ([]string, error) {
	rest := args
	for len(rest) > 0 {
...
func runLogsTail(opts logsOptions, filters []string, stdout, stderr io.Writer) int {
	...
	return followLogs(opts, filters, stdout, stderr, lastSeq(records))
}

func runLogsTrace(opts logsOptions, rest []string, stdout, stderr io.Writer) int {
...
func runLogsActivation(opts logsOptions, rest []string, stdout, stderr io.Writer) int {
	if len(rest) < 1 || strings.TrimSpace(rest[0]) == "" {
...
func runLogsStats(opts logsOptions, rest []string, stdout, stderr io.Writer) int {
	by, filters := parseLogStatsArgs(rest)
	records, err := query.Search(opts.dir, filters, time.Now().UTC())
	if err != nil {
		return fail(stderr, err)
	}
...
func parseLogStatsArgs(rest []string) (string, []string) {
	by := "event"
...
func followLogs(opts logsOptions, filters []string, stdout, stderr io.Writer, startSeq uint64) int {
	...
	for {
		...
		case <-ctx.Done():
			return 0
		case <-ticker.C:
			newRows, nextSeq, err := nextLogRows(opts, filters, currentSeq)
			currentSeq = nextSeq
			if err != nil {
				return fail(stderr, err)
			}
...
func nextLogRows(opts logsOptions, filters []string, currentSeq uint64) ([]query.Record, uint64, error) {
	records, err := query.Search(opts.dir, filters, time.Now().UTC())
	if err != nil {
...
func runTALTests(pkg appruntime.Package, testFiles []string) ([]talTestResult, error) {
	...
	for _, testPath := range testFiles {
		fileResults, err := runTALTestFile(pkg, mainContent, testPath)
...
func runTALTestFile(pkg appruntime.Package, mainContent, testPath string) ([]talTestResult, error) {
	testCases, parseErr := parseTALTestCases(testPath)
	if parseErr != nil {
		return []talTestResult{failedTALTestResult("parse", testPath, 1, parseErr)}, parseErr
	}
	results := make([]talTestResult, 0, len(testCases))
	var firstErr error
	for _, testCase := range testCases {
		result, err := runTALTestCaseResult(pkg, mainContent, testCase)
...
func runTALTestCaseResult(pkg appruntime.Package, mainContent string, testCase talTestCase) (talTestResult, error) {
	testErr := runTALTestCase(pkg, mainContent, testCase)
	if testErr != nil {
		return failedTALTestResult(testCase.Name, testCase.Path, testCase.Line, testErr), testErr
	}
	return talTestResult{
		Name:    testCase.Name,
		Path:    testCase.Path,
		Line:    testCase.Line,
		Passed:  true,
...
func failedTALTestResult(name, path string, line int, err error) talTestResult {
	return talTestResult{
		Name:    name,
		Path:    path,
		Line:    line,
...
func scanTALTestLine(path, rawLine string, lineNo int, current **talTestCase, currentIndent *int) (talScanAction, error) {
	...
	if trimmed == "" || strings.HasPrefix(trimmed, "#") {
		return talScanBreak, nil
	}
	indent := leadingSpaces(rawLine)
	if *current != nil && indent <= *currentIndent {
		return talScanAppendCurrent, nil
	}
	if *current == nil {
		next, err := parseTALTestDeclaration(path, trimmed, lineNo)
		if err != nil {
			return talScanBreak, err
		}
		*current = next
		*currentIndent = indent
		return talScanBreak, nil
	}
	assertion, err := parseTALTestAssertionLine(path, trimmed, lineNo)
	if err != nil {
		return talScanBreak, err
	}
...
func parseTALTestDeclaration(path, trimmed string, lineNo int) (*talTestCase, error) {
...
func parseTALTestAssertionLine(path, trimmed string, lineNo int) (talAssertion, error) {
	if !strings.HasPrefix(trimmed, "assert ") {
		return nil, fmt.Errorf("%s:%d expected assert statement", path, lineNo)
	}
	assertion, err := parseTALAssertion(strings.TrimSpace(strings.TrimPrefix(trimmed, "assert ")))
	if err != nil {
...
func parseTALAssertion(raw string) (talAssertion, error) {
	...
	case strings.HasPrefix(raw, "manifest.name == "):
		return parseManifestNameAssertion(raw)
	case strings.HasPrefix(raw, "manifest.version == "):
		return parseManifestVersionAssertion(raw)
	case strings.HasPrefix(raw, "main.contains(") && strings.HasSuffix(raw, ")"):
		return parseMainContainsAssertion(raw, true)
	case strings.HasPrefix(raw, "!main.contains(") && strings.HasSuffix(raw, ")"):
		return parseMainContainsAssertion(raw, false)
	default:
		return nil, fmt.Errorf("unsupported assertion %q", raw)
	}
}

func parseManifestNameAssertion(raw string) (talAssertion, error) {
...
func parseManifestVersionAssertion(raw string) (talAssertion, error) {
...
func parseMainContainsAssertion(raw string, positive bool) (talAssertion, error) {
	prefix := "main.contains("
	if !positive {
		prefix = "!main.contains("
	}
	want := parseQuotedValue(strings.TrimSuffix(strings.TrimPrefix(raw, prefix), ")"))
	if want == "" {
		return nil, fmt.Errorf("%smain.contains assertion requires quoted value", negationPrefix(positive))
	}
	return func(_ appruntime.Package, mainContent string) error {
		hasContent := strings.Contains(mainContent, want)
		if positive && !hasContent {
...
func negationPrefix(positive bool) string {
	if positive {
		return ""
	}
	return "!"

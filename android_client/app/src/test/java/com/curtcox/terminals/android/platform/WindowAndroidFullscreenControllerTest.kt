...
class WindowAndroidFullscreenControllerTest {
    @Test
    fun legacySystemUiVisibilityEnabledUsesImmersiveSystemBarsFlags() {
        ...
        assertEquals(expected, legacySystemUiVisibility(enabled = true))
    }
    ...
    fun legacySystemUiVisibilityDisabledUsesStableLayoutFlagOnly() {
        assertEquals(
            View.SYSTEM_UI_FLAG_LAYOUT_STABLE,
            legacySystemUiVisibility(enabled = false),
        )
    }
}

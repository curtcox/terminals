...
- Local stream starts now surface deterministic status when OS media permissions are denied (`Media permission required`), and no local offer is emitted for the denied stream.
- Video surface primitives now expose deterministic stream state (`Waiting for media` / `Attached`) so start/stop attach transitions are observable in widget coverage.
